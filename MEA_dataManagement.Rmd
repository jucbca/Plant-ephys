...
# run: python /Users/j/Documents/GitHub/MCS_dataAnalysis/MCS_dataManager.py
...
```{r}
setwd(home)
genotypes = c("slah3-3","slah3-1") #,,"slah3-3", "col0","msl10-1" "slah3-1",
for (g in genotypes){
  folders = list.dirs()[grep(g, list.dirs())]
...
for( p in unique(genotype_params_df$Parameter) ){
  ...
  ggplot( dplyr::filter(genotype_params_df, Parameter == p), #&Genotype=="col0"
    ...
    geom_smooth(aes(color=Genotype), fullrange = TRUE)+ #, se = FALSE
    ylab(p) +
    #ylim(0,80)+
    ...
    print(plot.p)
    ggsave(paste(p,"_scatterplot.pdf"))

}

# manually erase outliers. like durations above 150 s.
#genotype_params_df[which(genotype_params_df$Parameter=="hyperpol.halfmax.frame"&genotype_params_df$Value>=3000),1] = NaN
...
for ( n in names(genotype_params) ){
  print(n)
  electrodeN = data.frame("depDur" = genotype_params[[which(names(genotype_params)==n)]]$depDur, 
             "electrode" = n) 
  parameters = rbind(parameters, electrodeN)
}

parameters = separate(parameters, electrode, c("row", "column") )
write.csv(parameters, "depDur.csv", row.names = FALSE)
