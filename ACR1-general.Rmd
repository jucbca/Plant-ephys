...
# in the chunk during stimulation on.
  ...
  stimoff= which(trace$TimeOfDay=="F1")[2] + stimdelay 
   # plot(trace$mV)+abline(v=stimon)+abline(v=stimoff)
...
folders = grep("doseResp", list.files())
dates = c()
id = 1
...
parameters2plot = c("peak.depol",
                    ...
                    "tau")
for( v in which(names(datasummary)%in%parameters2plot) ){
figure <-   ggplot(dplyr::filter(datasummary), aes(treatment.time, datasummary[,v], color = `electrode` )) +
  geom_point(size = 3) +
