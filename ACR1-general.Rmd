...
NormalizeBaseline <- function(trace, normTime = 0 ){
  ...
  return(trace)
} ## INPUT trace spreaded i.e. each electrode as column. Won't work if data is not at 10Hz samplerate. Output: normalized, gathered trace for plotting, Voltage data transformed from V to mV!
...
# in the chunk during stimulation on.
  ...
  stimoff= which(trace$TimeOfDay=="F1")[2] + stimdelay 
   # plot(trace$mV, type="l")+abline(v=stimon)+abline(v=stimoff)
  stimchunk = na.omit(trace[stimon:(stimoff+10),])
  # plot(stimchunk$mV)
  ...
  if ( peakdepol.frame-stimon > 10 ){
    raisechunk = na.omit( trace[stimon:peakdepol.frame,] )
  } else {
    raisechunk = na.omit( trace[stimon: (peakdepol.frame+10) ,] )
  }
...
CheckSampleRate <- function(trace){
  sampleRate = round(60/as.numeric(trace$Time[60+1]),1) # 60 samples in x seconds that ran in the first 60 samples. (Hz)
  ...
  for (s in seq(1,length(stimuli),2) ){ # take every second mark, so the first of every stimulus
    ...
    for ( e in unique(tracein$electrode) ){
      trace.e = dplyr::filter(tracein, electrode==e)
        if ( s == 1 && length(stimuli)>2){ # pick different lengths depending of the order of the pulse
...
for( f in list.files()[grep(".txt",list.files())] ){ # 
  ...
  for(i in  names(trace)[3:(2+max(pickElectrodes))] ){
    electrodeNums = append(electrodeNums, as.numeric( substr(i,2, 2) ))
  }
  names(trace)[3:(2+max(pickElectrodes))] = electrodeNums # Make the columns' names numeric variables
  ...
  # last = max(which(trace$TimeOfDay=="F1")) 
  # plot(trace$mV[last:nrow(trace)])
  # 
  # which(trace$mV[last:nrow(trace)] < (-12) )
  # stim =last+   30520
  # plot(trace$mV[(stim-1000):(stim+2000)]) +
  #   abline(v=1000)
  # 
  # 
  # 
  # 
  # trace$TimeOfDay[stim] = "F1"
  ### write.csv(trace, "msl10-2.2.3.csv", row.names = FALSE)
...
f = list.files()[grep(".RData", list.files()) ][1]
print(f)
load( f )
...
for(i in c(1:(pulses.per.round)) ){
  for (e in pickElectrodes){
    trace.segmented[[i]][[e]]$Treatment = "baseline"#treatment##       #
    trace.segmented[[i]][[e]]$Treatment.time = 0
  }
}
for(i in c( (pulses.per.round+1)  : (pulses.per.round*2)  )){
    for (e in pickElectrodes){
    trace.segmented[[i]][[e]]$Treatment = "cut" # treatment## "petioleCut" #
    trace.segmented[[i]][[e]]$Treatment.time = 0
  }
}

time.initial = period_to_seconds(hms( trace.segmented[[1]][[1]]$TimeOfDay[1] )) 
counter = 1 # for what? - to count the pulses of the same round and give all the same time
for(i in c((pulses.per.round*2+1):length(trace.segmented)) ){
  if(counter <= pulses.per.round ){
    if(counter==1){
      time.pulse =round (period_to_seconds(hms(trace.segmented[[i]][[1]]$TimeOfDay[1])) -
        time.initial ,0 ) 
    }
    for (e in pickElectrodes){
    trace.segmented[[i]][[e]]$Treatment = treatment
    trace.segmented[[i]][[e]]$Treatment.time = time.pulse
  }
    print(time.pulse)
    counter = counter + 1
  }
...
for(f in files){
  ...
  pulse.to =length(names(trace.segmented)) # 1,2,3rd. input the number. or length(names(trace.segmented)) to do ALL.
  ...
  abline(v=onepulse$Time[which(onepulse$TimeOfDay=="F1")+1], col="red")
      ...
      to = max( which(onepulse$TimeOfDay=="F1") ) + 200# #nrow(onepulse) #  which(round(onepulse$Time,1)== 10)[1] # 
     
    
# if trace's absolute max peak is less than 2mV in amplitude:
   if( min( abs(na.omit( onepulse$mV[which(onepulse$TimeOfDay=="F1")[1]+stimulusDelay:
                          which(onepulse$TimeOfDay=="F1")[2]+stimulusDelay] ))*-1 ) - mean( na.omit( onepulse$mV[1:which(onepulse$TimeOfDay=="F1")[1]+stimulusDelay] )) > -0.0001 ) {
 # plot
      ...
      if( answer == "y" ){
        ...
        datasummary = rbind( datasummary, 
                       cbind( i,
                              ...
                              "treatment"=onepulse$Treatment[1],
                              "treatment.time"=onepulse$Treatment.time[1]
                              )
                      ) 
...
```{r}
#How many pulses per intensity?
ppi = 1#pulse per intentisty
intensity.values = c("0.5" = 49.89,
                     "1" = 96.6,
                     "2" = 218.68,
                     "3" = 347.13,
                     "4" = 713.38,
...
# # change intensity values of the complete summary. Change the dictionary then run this loop
# for( i in unique(DoseResponse_datasummary$Intensity) ){
#   DoseResponse_datasummary$Intensity[which(DoseResponse_datasummary$Intensity==i)] =
#     intensity.values[which(names(intensity.values) == i)]
# }
# write.csv(DoseResponse_datasummary, "DoseResponse.datasummary.csv",row.names = FALSE )
```
4.2
...
if( length(grep("RData",list.files()))>1 ){
...
datasummary = NULL
for( i in 1:(ppi*length(intensity.values)) ) {
  trace.joint[[i]]$'1'$Intensity = intensity.values[index.intensity]
  # keep 1st trace of that intensity for analysis
  if ( index4 == 1){ 
        # given interruption in the data aqcuisition time vector might restart. Identify if there is more than one 0 and make unique time vector 
    if( length(which(trace.joint[[i]]$'1'$Time==0)) >= 1 ){
          trace.joint[[i]]$'1'$Time[
      which(trace.joint[[i]]$'1'$Time==0):nrow(trace.joint[[i]]$'1')
      ] = trace.joint[[i]]$'1'$Time[
      which(trace.joint[[i]]$'1'$Time==0):nrow(trace.joint[[i]]$'1')
      ] + trace.joint[[i]]$'1'$Time[ (which(trace.joint[[i]]$'1'$Time==0)-2) ]
    }
...
# save datasummary
write.csv(datasummary, paste(savename,"-datasummary.csv",sep=""), row.names = FALSE)
...
ggplot( trace.plot, aes(Time, mV, group = Intensity, color = Intensity) )+ #dplyr::filter(trace.compilation, Intensity==c(1,2))
  ...
  # add stim:
  geom_vline(xintercept = trace.compilation[[1]]$Time[which(trace.compilation[[1]]$TimeOfDay=="F1")+10][1:2], 
             color = "darkolivegreen3", linetype = "dashed" ) +
...
4.3. Plot the dose response curve from the datasummary.csv file from 4'
input   - datasummary.csv files in their folder.
output  - Plot of calculated variables vs the duration of the stimulus in log scale. "DoseResponse_datasummary.csv" file.
```{r}
setwd(home)
# get all the summaries in the same data frame
folders = grep("msl", list.files())
dates = c()
id = 1
DoseResponse_datasummary = data.frame()
for (f in folders ){
  name = list.files()[f]
  date = substr(name,nchar(name)-5,nchar(name))
  setwd( paste(home,list.files()[f],sep="/") )
  datasummary = read.csv( list.files()[grep("datasummary",list.files())] )
  datasummary$date = date
  if (date %in% dates ){
    id = length(which(dates == date)) + 1
    datasummary$id = id
  } else {
    datasummary$id = 1
  }
  DoseResponse_datasummary = rbind( DoseResponse_datasummary, datasummary )
...
#                     "peakdepol.latency",
#                     "peak.depol.slope",
for( v in which(names(DoseResponse_datasummary)%in%parameters2plot) ){
figure <-   
  ggplot(dplyr::filter(DoseResponse_datasummary), aes(`Intensity`, DoseResponse_datasummary[,v], color = `Leaf` )) + #
  geom_point(size = 3) +
...
4.4. 
input - DoseResponse.datasummary.csv for one genotype
output - scatterplots of the parameters normalized and with mean SEM whiskers. 

```{r}
setwd(home)
data = read.csv("DoseResponse.datasummary.csv")
plot.parameters = c("peak.depol",
                    "peak.depol.slope",
                    "peakdepolslope.latency",
                    "tau")
# modify data: normalize to max peak.depol of each leaf.
normalized.data = NULL
for( d in unique(data$date) ){
  set <- data %>% dplyr::filter(date == d)
  print(d)
  for( i in unique(set$id) ){
    set1 <- set %>% dplyr::filter(id==i)
    print(paste("id",i))
    for (p in which(names(data) %in% plot.parameters) ){
      set.max = max(abs( set1[,p] ))
      set1[,p] = abs( set1[,p] ) /as.numeric(set.max)
      print(names(data)[p])
    }
    normalized.data = rbind( normalized.data, set1)
  }
}
# get fit
...
for ( i in unique(normalized.data$Intensity) ){
  set1 = dplyr::filter(normalized.data, Intensity==i)
  set.summary = data.frame("Intensity" = i )
  for (p in which(names(data) %in% plot.parameters) ){
    summarized.data = rbind( summarized.data, cbind( set.summary, data.frame(
      "Mean" = mean(na.omit(set1[,p])) ,
      "SEM" =sd( na.omit(set1[,p])/nrow(set1)) ,
      "n" =  length(na.omit(set1[,p]) ),
      "Parameter" = names(data)[p],
      "Genotype" = "Col0"
    ) )
    )
  }
...
for ( p in plot.parameters){
  ggplot(dplyr::filter(summarized.data, Parameter==p ),
       aes(log(Intensity,base=10), Mean) ) +
  geom_point(size = 1.5, color = "magenta") +
  geom_errorbar(aes(ymin = Mean - SEM, ymax = Mean+SEM), color="magenta") +
  geom_point(data=normalized.data, aes(log(Intensity,base=10), normalized.data[,which(names(normalized.data)==p)]),
             size = 0.5, alpha = 0.6) + 
  ggtitle( p ) +
  ylab (paste("Normalized ", p, sep = "")) +
  xlab("log10(Light intensity µW/mm2) ") +
  theme(
...
########## fit data
# transform data
x = log(normalized.data$Intensity, base = 10)
y = normalized.data$peak.depol
#  use a linear model.  lm()
fit <- lm(y ~ ns(x,i) )
fit.info = summary(fit)
fitR2 = fit.info$r.squared
### OOOOOR 
# Use a non-linear model. nsl()
fit <- nls( y ~ SSlogis(x, Asym, xmid, scal) ) 
fit.info = summary(fit)
# add to dataframe
normalized.data <- add_predictions(normalized.data, fit)
# plot
ggplot(normalized.data, aes( log(Intensity,base=10), peak.depol ))+
  geom_point() +
  geom_line( aes( y = pred ), color="red" )

# add fitting to df and save
ggsave("fitting-peak.depol.pdf")
names(normalized.data)[18] = "LogisticRegression.Fit"
write.csv(normalized.data, "Normalized.peak.depol.csv", row.names = FALSE)
save(fit.info,file="fitting-peak.depol-parameters.RData")
...
4.5. Genetic background - ACR1 construct comparison.
input - A "DoseResponse_datasummary_>genotype<.csv" file with all analyzed statistics of each genotype
output- 

```{r}
setwd(home)

compiled_datasummary = NULL
for (f in list.files()[grep("datasummary",list.files())]) {
  data = read.csv(f)
  compiled_datasummary = rbind(compiled_datasummary, data )
  print(f)
}
write.csv(compiled_datasummary, "genotype_datasummary_240726.csv")
# Plot all parameter with both genotypes
for( v in which(names(DoseResponse_datasummary)%in%parameters2plot) ){
figure <-
  ggplot(dplyr::filter(compiled_datasummary), aes(`Intensity`, compiled_datasummary[,v], color = `Genotype` )) + #
  geom_point(size = 2) +
  ...
  My_Theme
  print(figure)
  ggsave(paste(home,"/genotypes_DoseResp-",names(datasummary)[v],".pdf",sep = "") )
...
# datasummary = dplyr::filter(datasummary, treatment=="Glu100mM" & electrode==2)
# datasummary = datasummary[,1:11]
# datacompilation = rbind(datacompilation, datasummary)
# datacompilation = datacompilation[grep("Col",datacompilation$Genotype),]
# write.csv(datacompilation, "datacompilation.csv", row.names = FALSE)
...
parameters2plot = c("peak.depol",
                    ...
                    "tau")
for( v in which(names(datasummary)%in%parameters2plot) ){
figure <-   ggplot(dplyr::filter(datasummary), aes(Genotype, datasummary[,v] )) +
  geom_boxplot()+
  geom_jitter( position=position_jitter(seed=3,width=0), 
               size = 3, shape=1) +
  #geom_point(size = 3) +
  ...
  xlab ("") + 
  ylab (names(datasummary)[v]) + 
  ggtitle(paste( unique(datasummary$treatment)[3]," treatment", " Power-6", sep="") ) +
  theme(
  ...
  axis.title.y = element_text(color = "black", size = 25, angle = 90, hjust = 0.5, vjust = 1, face = "plain"),
  plot.title = element_text(color = "black", size = 10, angle = 0, hjust = 0.5, vjust = 0, face = "plain"), # element_blank(), 
  legend.text = element_text(size = 20),
  legend.title = element_text(hjust = 0.1, size = 20),
  panel.grid.major.y = element_blank(),
  panel.grid.minor.y = element_blank(),
