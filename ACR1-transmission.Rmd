...
# in the chunk during stimulation on.
  ...
  peak.depol =  min( raisechunk$mV )
  peakdepol.frame =  max(which(trace$mV==peak.depol)-1)
  peakdepol.time =  trace$Time[peakdepol.frame]
  peakdepol.latency = peakdepol.time - trace$Time[stimon]
  
  plot(raisechunk$Time, raisechunk$mV, type="p") 
...
```{r}
stimulusDelay = 0# the time between light onset and the computer making the mark.
```
...
for( f in list.files()[grep(".txt",list.files())] ){ # 
  ...
  if ( intSplit[[1]][2] == "." ){
    intensity = as.numeric(intensity) + 6
  } else {
...
input   -  >genotype.RData< traces in the batch folder. The list is called "trace.segmented"
...
for(f in files){
  ...
  for(i in 1:length(names(trace.segmented)) ){
    ...
    onepulse$Time = onepulse$Time-onepulse$Time[which(onepulse$TimeOfDay=="F1")[1]+stimulusDelay] # set the stimulus onset as time = 0 
  ...
  abline(v=onepulse$Time[which(onepulse$TimeOfDay=="F1")+1], col="red")
      ...
      from = which(round(onepulse$Time,1)==(-0.1))[1]
...
# if trace's absolut max peak is less than 2mV in amplitude:
   if( min( abs( onepulse$mV[which(onepulse$TimeOfDay=="F1")[1]+stimulusDelay:
                          which(onepulse$TimeOfDay=="F1")[2]+stimulusDelay] )*-1 ) - mean( onepulse$mV[1:which(onepulse$TimeOfDay=="F1")[1]+stimulusDelay] ) > -0.1 ) {
 # plot
...
trace.compilation = NULL #data.frame( "Time" = trace.segmented[[1]]$'1'$Time)
datasummary = NULL
# Loop through all first traces. provisional!
for( f in grep("RData",list.files()) ){ # go through the lists of the different experiment
  print(list.files()[f])
  load(list.files()[f])
  print(trace.segmented[[1]]$'1'$Intensity[1] )
  savename = substr(list.files()[f], 1, 6) 
  #savename = list.files()[f] %>% substr(1,nchar(list.files()[f])-6)
  # normalize time to start at 0 for that pulse
  trace.segmented[[1]]$'1'$Time = trace.segmented[[1]]$'1'$Time-trace.segmented[[1]]$'1'$Time[1]
  # normalize voltage to that 1 s before the pulse
  trace.segmented[[1]]$'1'$mV = trace.segmented[[1]]$'1'$mV - 
    trace.segmented[[1]]$'1'$mV[ which(trace.segmented[[1]]$'1'$TimeOfDay  == "F1")-10 ][1]
  # plot and save that trace
  pdf( paste(batchfolder,"/PulsePlots/",savename,".pdf",sep="") )
  plot(trace.segmented[[1]]$'1'$Time , trace.segmented[[1]]$'1'$mV, "l")
  dev.off()
  # save the first pulse. organize as time, I1,I2...In,leaf,id,genotype
  trace.compilation = rbind(trace.compilation, data.frame( "Time" = trace.segmented[[1]]$'1'$Time,
                                                           "mV" = trace.segmented[[1]]$'1'$mV,
                                                           "Intensity" = trace.segmented[[1]]$'1'$Intensity[1] ) )
  
  leaf = trace.segmented[[1]]$'1'$Leaf[1]
  id = trace.segmented[[1]]$'1'$id[1]
  genotype = trace.segmented[[1]]$'1'$Genotype[1]
  
  
  #plot( trace.segmented[[1]]$'1'$Time, trace.segmented[[1]]$'1'$mV,)
  
  datasummary = rbind(datasummary,
                      cbind( as.data.frame( CalculateParameters( trace.segmented[[1]]$'1',stimdelay=0)),
                             "Intensity" = trace.segmented[[1]]$'1'$Intensity[1],
                             "Leaf" = trace.segmented[[1]]$'1'$Leaf[1],
                             "id" = trace.segmented[[1]]$'1'$id[1],
                             "Genotype" = trace.segmented[[1]]$'1'$Genotype[1] ) 
...
trace.compilation$Intensity = as.character(trace.compilation$Intensity)
trace.compilation$Intensity = as.numeric( replace(trace.compilation$Intensity, trace.compilation$Intensity %in% names(intensity.values), intensity.values[trace.compilation$Intensity]) )
datasummary$Intensity = as.character( datasummary$Intensity )
datasummary$Intensity = as.numeric( replace(datasummary$Intensity , datasummary$Intensity  %in% names(intensity.values), intensity.values[datasummary$Intensity ]) )
# save datasummary
write.csv(datasummary, "Col0-1-datasummary.csv")

### Plot overlay of pulses
ggplot( trace.compilation, aes(Time, mV, group = Intensity, color = Intensity) )+ #dplyr::filter(trace.compilation, Intensity==c(1,2))
  geom_line() + 
...
```{r}
setwd(batchfolder)
datasummary$peakdepolslope.latency[which(datasummary$peakdepolslope.latency>=0.2)] = NaN
...
figure <-   ggplot(dplyr::filter(datasummary), aes(`stim.duration.ms.`, datasummary[,v], color = `position` )) +
  geom_point(size = 3) +
  ...
  ggtitle("Dose-response.Power-1") +
  My_Theme
print(figure)
#ggsave(paste(batchfolder,"/DoseResponse-",names(datasummary)[v],".pdf",sep = "") )
}
