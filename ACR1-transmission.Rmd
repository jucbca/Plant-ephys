...
output  - datasummary.csv file with all the parameters for each stimulus just for electrode 1
```{r}
setwd(batchfolder)
datasummary = NULL
if(file.exists( paste(batchfolder,"/PulsePlots",sep=""))==FALSE){
...
for( f in grep("RData",list.files()) ){ # go through the lists of the different experiment
  ...
  for(i in 1:length(trace.segmented) ) { # loop through the stim times
    ...
    head(trace.segmented[[i]]$'1') ## this might not work for both electrodes

    one.stimulus = cbind( "stim.duration.ms." =  as.numeric( strsplit(names(trace.segmented)[i],split = " ")[[1]][1] )  ,
                        "electrode" = trace.segmented[[i]]$'1'$electrode[1],
                        "stimulus" = trace.segmented[[i]]$'1'$Stim[1] , 
                        data.frame( CalculateParameters( trace.segmented[[i]]$'1',stimdelay=0) ),
                        "genotype" = trace.segmented[[i]]$'1'$Genotype
    ...
    #### ACA VA. algo no funciona con calculate parameters!!
    
    
    
    
    if(is.null(trace.segmented[[i]][[j]]$Treatment[1]) == FALSE ){
      one.stimulus = cbind(one.stimulus,
                           "treatment" = trace.segmented[[i]][[j]]$Treatment[1],
                           "time.treatment" = trace.segmented[[i]][[j]]$Treatment.time[1])
    } 
    
    #plot the stim segment
    ...
    plot(trace.segmented[[i]][[j]]$Time, trace.segmented[[i]][[j]]$mV,
      ...
      abline(v=one.stimulus$peakdepolslope.time, col="green", lty=2) +
