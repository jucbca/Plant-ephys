...
input   -  >savename.RData< traces separated by pulses.
...
ggplot( trace.compilation, aes(Time, mV, group = Intensity, color = Intensity) )+ #dplyr::filter(trace.compilation, Intensity==c(1,2))
  geom_line() + 
  scale_color_gradient( low="black", high="darkolivegreen3") +
  # add stim:
  geom_vline(xintercept = trace.compilation$Time [which( is.na(trace.compilation$Time) == TRUE )-1][1:2], color = "darkorange", linetype = "dashed" )
